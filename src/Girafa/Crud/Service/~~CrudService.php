...
class CrudService implements CrudServiceInterface, ServiceLocatorAwareInterface {
    ...
    public function saveEntity($I_entity, $attempt = 1){
    	...
    	try {
    		...
    		$attempt++;
    		if($attempt <= 3) {
    			$this->saveEntity($I_entity, $attempt);
    		}
    ...
    public function deleteEntity($I_entity, $attempt = 1) {
    	$conn = $this->I_entityManager->getConnection();
    	try {
    		$conn->beginTransaction();    		
    		...
    		$attempt++;
    		if($attempt <= 3) {
    			$this->deleteEntity($I_entity, $attempt);
    		}
    		throw new \Exception($e->getMessage(), $e->getCode());
    	}
